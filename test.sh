...
runTest() {
  ...
  echo "Test $TESTNO: ./kiosk ${args[@]} < input-$TESTNO.txt > output.txt 2> stderr.txt"
  ./kiosk ${args[@]} < input-$TESTNO.txt > output.txt 2> stderr.txt
  STATUS=$?
...
if [ -x kiosk ] ; then
    ...
    args=(menu-a.txt menu-b.txt menu-c.txt)
    runTest 13 0
    ...
    args=(menu-a.txt file-that-doesnt-exist.txt)
    runTest 16 1
